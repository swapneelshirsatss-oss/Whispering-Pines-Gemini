...
export default function Blog() {
  ...
  const activePost = BLOG_POSTS.find((p) => p.slug === selectedPost);
  ...
  useEffect(() => {
    ...
    if (hash) {
      const found = BLOG_POSTS.find(p => p.slug === hash);
      if (found) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {BLOG_POSTS.map((post) => (
              <article key={post.id} className="bg-white rounded-lg shadow-sm border border-[#EAE8E3] overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                <div className="h-48 w-full overflow-hidden cursor-pointer" onClick={() => handlePostClick(post.slug)}>
                  <img 
                    src={post.imageUrl} 
                    alt={post.title} 
